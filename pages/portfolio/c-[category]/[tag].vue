...
import {fetchCategoriesPortfolio, fetchPortfolioSettings, fetchTagsPortfolio} from "~/composables/cmsPortfolio";
...
const tagActive = route.params.tag
...
const fetchedTags = await fetchTagsPortfolio()
...
const tagFilter = fetchedTags.filter(c => c.slug == tagActive);
...
try {
    projects = await fetchPortfolioPreview(page.default_output_count, catFilter[0].id, tagFilter[0].id,)
} catch (e) {
    console.log(e)
    throw createError({statusCode: 404, statusMessage: 'Page Not Found'})
}
