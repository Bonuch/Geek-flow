<template>
    <div class="portfolio_page">
        <div class="portfolio_block__head">
            ...
            <CategoryFilter :active-category="categories[0].value" @change-category="change_category" :categories="categories"/>
        </div>
        <div>
...
const projects = await fetchPortfolioPreview(page.default_output_count)
...
fetchedCats.forEach(c => {
    categories.push({name: c.name, value: c.slug})
})
const route = useRoute()
const change_category = (slug) => {
    if (slug != 'uncategorized') {
        navigateTo(route.fullPath+'/c-'+slug)
    }
}
</script>
